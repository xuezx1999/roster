...
## D14. 中英文间距：不自动插入窄空格，由用户手动输入（撤销 0.9.9 的 AutoSpace）

**现状**：0.9.9 曾引入"显示层自动在中英文之间插入 0.25em 窄空格"（`AutoSpace.tsx`），0.9.12 撤销，恢复原文直接渲染，中英文之间如需留白由用户手动输入半角空格（`helpContent` 与教程列表的手动空格一并恢复）。
**理由**：以**视觉一致性**为优先——应用是等宽字体（IBM Plex Mono），英文单词之间的空格是等宽半角（≈0.5em）；0.25em 自动窄空格比单词间空格**更小**，同一行内出现两种"空格"尺寸会造成视觉逻辑不一致。用户手动输入的半角空格与英文单词间空格等宽，视觉统一、可接受。
**代价**：中英文紧贴（如"完成Roster任务"）时需用户手动补空格；英文/数字等紧贴中文处依赖用户书写习惯，不再自动美化。
**何时可改**：当应用字体栈改为非等宽（比例字体）且希望遵循中文排版规范（中英间加薄空格）时，可重新引入——届时需保证间距宽度与字体默认词间距视觉一致，而非固定 0.25em。
