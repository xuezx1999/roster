...
### 变更
- **回滚 0.9.9 的中英文混排自动窄空格功能**：任务内容、列表标题、使用说明页恢复为原文直接显示，不再自动插入 `0.25em` 窄空格；中英文之间如需留白由用户手动输入半角空格。删除 `src/components/AutoSpace.tsx`；`TaskItem`/`EditableTitle`/`HelpPage` 恢复 0.9.8 渲染方式（HelpPage 保留 0.9.11 的两端对齐）；`helpContent.ts` 与教程列表（`buildDemoList`）恢复手动空格。**保留**：NO TASKS 空态提示（0.9.10）、使用说明两端对齐（0.9.11）。存储数据从未被自动空格污染（原功能仅显示层生效），无需数据迁移。

### 文档同步
- `package.json` version `0.9.11 → 0.9.12`。
- `src/helpContent.ts` 使用说明「当前版本」0.9.11 → 0.9.12；主要更新内容替换为本版说明；正文恢复手动空格。
- `docs/DESIGN_SYSTEM.md` §3 移除中英文窄空格规范（保留两端对齐）。
- `docs/AI_CONTEXT.md` 当前项目状态版本号 0.9.11 → 0.9.12。

---

## [0.9.11] 2026-08-13 — 使用说明正文两端对齐排版

### 变更
- **使用说明正文改为两端对齐（text-justify）**：段落、引用、列表项统一 `text-justify`（列表/引用项的文本 span 加 `flex-1 min-w-0` 占满行宽，否则 flex 布局下文本不填满行、justify 不生效）。中文方块字两端对齐后左右边缘整齐，符合中文正文排版习惯；标题保持左对齐（单行无效果）。涉及 `HelpPage.tsx` `Blocks` 组件。

### 文档同步
- `package.json` version `0.9.10 → 0.9.11`。
- `src/helpContent.ts` 使用说明「当前版本」0.9.10 → 0.9.11；主要更新内容替换为本版说明（延续"只保留最新一版"约定）。
- `docs/DESIGN_SYSTEM.md` §3 排版补充使用说明正文两端对齐。
- `docs/AI_CONTEXT.md` 当前项目状态版本号 0.9.10 → 0.9.11。
...
## [0.9.4] 2026-08-12 — 使用说明页移动端可横向滑动修复

### 修复
- **移动端使用说明页可左右滑动（内容外为空白，P1）**：`HelpPage.tsx` 正文滚动容器只声明 `overflow-y-auto`——按 CSS 规范，一个轴为 `auto` 时另一轴的 `visible` 会被计算为 `auto`，使 x 方向同样允许滚动（iOS 表现为左右橡皮筋滑动，滑到内容外是空白）。修复：滚动容器补 `overflow-x-hidden`，与列表页单列表容器（`overflow-x-hidden`）同一禁横滚策略；页面仅保留上下滚动。

### 文档同步
- `package.json` version `0.9.3 → 0.9.4`。
- `helpContent.ts` 使用说明「当前版本」0.9.3 → 0.9.4。

---

## [0.9.3] 2026-08-12 — 移动端空任务列表 NO LISTS 垂直居中修复

### 修复
- **移动端空任务列表 NO LISTS 未垂直居中于页面（P1）**：`App.tsx` 空任务列表分支（列表存在但 tasks 为空）用 `min-h-[40vh]`，空态只占视口 40% 高度、且位于外层 `paddingTop: safe+108` 之后，NO LISTS 居中于这个小盒子导致整体严重偏上（852px 视口下偏上 ~148px）；桌面 ListPanel 同分支用 `h-full`（继承视口高度链）居中，故仅移动端可见。修复：改为 `min-h-[calc(100svh-env(safe-area-inset-top)-env(safe-area-inset-bottom)-236px)]`（236 = 108 顶部留白 + 128 底部留白，占满外层 padding 间剩余空间），与桌面 ListPanel / 无列表空态同一居中基准（中心 ≈ 50svh − 64，三者偏差 ≤4px 为有意留白差）；保留 `min-h-[40vh]` 兜底（iOS < 16.4 不支持 100svh 时回退原行为，不算回归）。

### 文档同步
- `package.json` version `0.9.2 → 0.9.3`。
- `helpContent.ts` 使用说明「当前版本」0.9.2 → 0.9.3。

---

## [0.9.2] 2026-08-12 — 自动导出间隔 24h → 7 天 + 导出提示横幅

### 变更
- **自动导出间隔 `24 小时 → 7 天`**：桌面端启动时距上次导出（`localStorage['last-export']` 时间戳）>7 天且列表非空才自动下载 JSON 备份，降低下载频率。
- **自动导出提示横幅**：自动导出成功时顶部显示黑底白字提示「已自动导出备份」（样式与 PWA 安装引导一致，`fixed top: safe+18px`，5 秒自动消失或点击 ✕ 关闭）；与 saveError / 新版本 / PWA 引导横幅互斥显示。
- **触屏环境不再消耗导出机会（修复）**：原逻辑先更新 `last-export` 时间戳、后检查 `pointer: fine`——触屏/非精细指针环境会更新时间戳却不下发导出（表现为"设置 8 天前时间戳后刷新无效果"）。改为 `pointer: fine` 检查通过后才更新时间戳并导出。
- **横幅退出淡出**：横幅自动消失/点击关闭时用 framer-motion exit `opacity 0.15s` 淡出（符合 DESIGN_SYSTEM §7 动画规格）。

### 文档同步
- `helpContent.ts` 使用说明「数据存储」自动导出描述更新（24 小时 → 7 天 + 提示说明），「当前版本」区块 0.9.1 → 0.9.2。
